...
## Database Rules
- SQLite setup lives in `lib/data/database.dart`; repositories live in `lib/data/repositories/`.
...
2. Add parser expectations for date, total, VAT, items, merchant, and categories.
3. Keep normalization rules conservative; do not break existing Biedronka/Polish receipt samples.
4. Use `closeTo` for monetary assertions.
