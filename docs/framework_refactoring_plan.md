...
- Зафиксировать E2E UI automation strategy:
  ...
  - Maestro: selected manual Android-native E2E lane for real native surfaces such as permissions, SAF/file picker, system dialogs, or native PDF/OCR paths;
  - Patrol is not adopted for this lane: its selected `file_picker` result does not reach Dart under the project JUnit runner; no app seam is warranted;
  - do not promote Maestro to routine CI until a separately scoped physical-device and CI proof is complete.
...
| Phase 5: ReceiptMonth value object | complete | `docs/refactoring_subplans/phase_5_receipt_month_value_object.md` | 2026-06-08 | Month normalization/mapping centralized in `ReceiptMonth`; focused tests, analyzer, and full suite passed. |
...
| Phase 10: E2E UI automation strategy | complete | `docs/refactoring_subplans/phase_10_e2e_ui_automation_strategy.md` | 2026-08-28 | Widget state coverage and six deterministic Android `integration_test` journeys are complete; Phase 16 selected Maestro for manual native Android scenarios. |
| Phase 15: Android device E2E automation track | in progress | `docs/refactoring_subplans/phase_15_android_device_e2e_track.md` | 2026-08-30 | Flutter E2E foundation checkpoint complete; the manual workflow remains outside PR/coverage gates. Phase 16 selected Maestro and Phases 17--19 supplied native-picker, navigation, month-isolation, and external-source-return evidence. Connected-device evidence remains a separately scoped follow-up. |
| Phase 15: E2E track reconciliation | complete | `docs/refactoring_subplans/phase_15_e2e_track_reconciliation.md` | 2026-08-30 | Active Android E2E documentation synchronized: Maestro is selected, Patrol is unsupported outside diagnosis, and the Flutter integration workflow remains distinct from the manual Maestro lane. |
| Phase 16: native E2E framework learning spike | complete | `docs/refactoring_subplans/phase_16_native_e2e_framework_learning_spike.md` | 2026-08-28 | Maestro is selected as the primary native Android E2E framework. It passed the native import and exact-duplicate scenarios twice; Patrol's selection-result limitation is recorded without adding an app seam. |
| Phase 16a: E2E receipt fixture corpus | superseded | `docs/refactoring_subplans/phase_16a_e2e_receipt_fixture_corpus.md` | 2026-08-27 | Folded into Phase 16e, which needs one approved derivative for comparable Patrol and Maestro scenarios. |
| Phase 16b: Patrol document-picker pilot | complete | `docs/refactoring_subplans/phase_16b_patrol_document_picker_pilot.md` | 2026-08-27 | Patrol Pilot A passed twice through the project JUnit helper on `it_api36`; the official CLI's host-local UTP result-listener TLS constraint remains documented and does not replace the JUnit result. |
| Phase 16c: Maestro document-picker pilot | complete | `docs/refactoring_subplans/phase_16c_maestro_document_picker_pilot.md` | 2026-08-27 | Black-box picker-cancel flow passed twice headlessly on `it_api36` with three stable Semantics IDs; use `-gpu host` with `-no-window` to avoid this host's System UI ANR. |
| Phase 16d: selected framework real-format receipt pilot | superseded | `docs/refactoring_subplans/phase_16d_selected_framework_real_receipt_pilot.md` | 2026-08-27 | Superseded by Phase 16e: both candidates will run the two real-PDF scenarios before selection. |
| Phase 16e: dual-framework redacted PDF pilots | complete | `docs/refactoring_subplans/phase_16e_dual_framework_redacted_pdf_pilots.md` | 2026-08-28 | Maestro passed successful native import and exact duplicate twice with the approved synthetic fixture and was selected. Patrol opens and selects the same file, but `file_picker`'s result never reaches Dart under its JUnit runner; that comparison outcome closes the parity requirement without an app test hook. |
| Phase 17: Maestro post-import navigation coverage | complete | `docs/refactoring_subplans/phase_17_maestro_post_import_navigation.md` | 2026-08-28 | The approved `receipt_a` native-picker journey reaches safe non-empty Receipts and Month outcomes twice on the verified headless AVD. Minimal Semantics IDs, focused tests, analyzer, and full Flutter tests pass; a second fixture is the next coverage package. |
| Phase 18: Maestro month-isolation coverage | complete | `docs/refactoring_subplans/phase_18_maestro_month_isolation.md` | 2026-08-29 | Owner-approved fully synthetic second-month fixture added. Maestro imported both through the native picker and proved one receipt in each ordinal Month option twice on `it_api36`; focused tests, analyzer, and full Flutter suite passed. |
| Phase 19: Maestro open receipt source smoke | complete | `docs/refactoring_subplans/phase_19_maestro_open_receipt_source.md` | 2026-08-30 | Reused `receipt_a` to prove Android external-source opening through one safe Back-to-Receipt-Details assertion twice on `it_api36`; focused tests, analyzer, and full Flutter suite passed. |
| Feature: delete receipt | complete | `docs/refactoring_subplans/phase_11_delete_receipt.md` | 2026-08-26 | Confirmed per-receipt deletion from Receipt Details with atomic aggregate maintenance, safe localized feedback, and passing repository/widget tests. |
| Feature: clear receipt data | complete | `docs/refactoring_subplans/phase_12_clear_receipt_data.md` | 2026-08-26 | Confirmed Settings cleanup of receipt history and aggregates while preserving configuration and seed metadata; repository and widget coverage pass. |
| Feature: re-categorize receipt line item | complete | `docs/refactoring_subplans/phase_13_recategorize_line_item.md` | 2026-08-26 | Per-line-item built-in category picker with transactional aggregate maintenance, localized current-category labels, and passing repository/widget coverage. |
| Feature: open receipt source | complete | `docs/refactoring_subplans/phase_14_open_receipt_source.md` | 2026-08-26 | Platform-backed opening of persisted receipt source URIs with safe local-only handling; Android Manifest and Kotlin compile checks pass, manual viewer smoke remains recorded. |
...
| Phase 10: analyze deduplication | complete | `docs/refactoring_subplans/phase_10_analyze_deduplication.md` | 2026-06-21 | `flutter analyze` now belongs to Android debug workflow only; Sonar workflow keeps coverage helper and scan; local analyzer/tests passed. |
